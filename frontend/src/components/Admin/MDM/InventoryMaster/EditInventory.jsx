...
export default function EditInventory() {
  ...
  const equipmentId = decryptData(new URLSearchParams(location.search).get('s'));

  async function fetchInventoryDetailsById() {
    try {
      let response = await axiosHttpClient('VIEW_INVENTORY_BY_ID_API', 'get', {}, equipmentId);
      console.log('response of fetched inventory details', response.data.data);
      let { code, description, statusId } = response.data.data;
      setFormData({ code, description, statusId, equipmentId });
    }
    catch(error) {
      console.error(error);
    }
  ...
  let handleSubmit = async () => {
    ...
    try {
      let res = await axiosHttpClient('UPDATE_INVENTORY_API', 'put', formData);
    ...
    <div>
      ...
      <div className="CreateInventoryContainer">
        <div className="form-heading">
          <h2>Edit/View Inventory details</h2>
          ...
          <div className="grid grid-rows-2 grid-cols-2 gap-x-16 gap-y-2 w-[100%]">
            <div className="form-group col-span-1">
              <label htmlFor="input2">Inventory Code <span className='text-red-500'>*</span></label>
            ...
            <div className="form-group col-span-1">
              <label htmlFor="input2">Inventory Description <span className='text-red-500'>*</span></label>
