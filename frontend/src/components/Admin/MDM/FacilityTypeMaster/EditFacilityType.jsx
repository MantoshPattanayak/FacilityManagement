...
export default function EditFacilityType() {
  ...
  const facilityTypeId = decryptData(new URLSearchParams(location.search).get('s'));

  async function fetchInventoryDetailsById() {
    try {
      let response = await axiosHttpClient('VIEW_FACILITYTYPE_BY_ID_API', 'get', {}, facilityTypeId);
      console.log('response of fetched inventory details', response.data.data);
      let { code, description, statusId } = response.data.data;
      setFormData({ code, description, statusId, facilityTypeId });
    }
    catch(error) {
      console.error(error);
    }
  ...
  let handleSubmit = async () => {
    ...
    try {
      let res = await axiosHttpClient('UPDATE_FACILITYTYPE_API', 'put', formData);
        ...
        <div className="form-heading">
          ...
          <div className="grid grid-rows-2 grid-cols-2 gap-x-16 gap-y-2 w-[100%]">
            <div className="form-group col-span-1">
              <label htmlFor="input2">Asset Code <span className='text-red-500'>*</span></label>
            ...
            <div className="form-group col-span-1">
              <label htmlFor="input2">Asset Description <span className='text-red-500'>*</span></label>
