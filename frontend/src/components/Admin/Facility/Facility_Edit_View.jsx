...
const Facility_Edit_View = () => {
  ...
  console.log("Decrypted facilityTypeId11: ", facilityTypeId);
  ...
  const handleChange = (e) => {
    ...
    // Handle selection change for facility is owned by BDA
    else if (name === "facilityisownedbBDA") {
      const isOwnerByBDA = value === "1"; // Use "1" for "Yes" as per your <option> values
      setPostFacilityData((prevData) => ({
        ...prevData,
        ownersAddress: {
          ...prevData.ownersAddress,
          [name]: value,
        }
      }));
    ...
    // Handle changes within facilityData
  ...
  async function DisplayFacilityData() {
    try {
      let res = await axiosHttpClient(
        "Facility_Update_View_By_ID_Api",
        "post",
        {
          facilityId,
          facilityTypeId,
      ...
      const fetchDataDisplayFacility = {
        ...
        fileNames: {
          facilityImageOne: {
            ...facilityImageOne[0],
            fileId: facilityImageOne[0]?.fileId || null,
          },
          facilityArrayOfImages:
            facilityArrayOfImages.map((image) => ({
              ...image,
      ...
      let postFacilityDetails = deepCopy(fetchDataDisplayFacility);
      // for (let key of Object.keys(fetchDataDisplayFacility)) {
      //   // console.log("here fetch data", Object.keys(fetchDataDisplayFacility))
      //   // if not array type
      //   if (typeof fetchDataDisplayFacility[key] === "object"
      //     && !Array.isArray(fetchDataDisplayFacility[key])) {
      //     postFacilityDetails[key] == JSON.parse(JSON.stringify(fetchDataDisplayFacility[key]));
      //   }
      //   // if array type
      //   else if (typeof fetchDataDisplayFacility[key] === "object"
      //     && Array.isArray(fetchDataDisplayFacility[key])) {
      //     console.log("fetchDataDisplayFacility[key]", fetchDataDisplayFacility[key]);
      //     let deepcopyDetails = fetchDataDisplayFacility[key].forEach((initialData) => {
      //       if (Array.isArray(initialData)) { // if object -> array type
      //         let nestedDeepcopyDetails = initialData.forEach((data) => {
      //           return JSON.parse(JSON.stringify(data));
      //         })
      //       }
      //       else {  // if not array type, then return
      //         return initialData;
      //       }
      //     });
      //     postFacilityDetails[key] = deepcopyDetails;
      //   }
      //   // if number, string type
      //   else if (typeof fetchDataDisplayFacility[key] !== "object"){
      //     /* postFacilityDetails[key] = JSON.parse(JSON.stringify(fetchDataDisplayFacility[key])); */
      //     postFacilityDetails[key] = fetchDataDisplayFacility[key];
      //   }
      // }
      console.log("here data copy ", postFacilityDetails)
      setPostFacilityData(postFacilityDetails);
      // Store initial facility data
      setinitialFacilityDataStore(JSON.parse(JSON.stringify(fetchDataDisplayFacility)));
    } catch (err) {
  ...
  const getChnageValues = (initialData, currentData) => {
    ...
    for (const key in currentData) {
        if (key === "parkInventory") {
        ...
        } else if (key === "facilityArrayOfImages") {
            // Handle facilityArrayOfImages separately
            const changedImages = currentData[key].filter((image, index) => {
                const initialImage = initialData[key]?.[index];
                // Image is new or has changed data
                return (
                    image.fileId === null || 
                    (initialImage && (!isEqual(initialImage.data, image.data) || !isEqual(initialImage.code, image.code)))
                );
            });

            if (changedImages.length > 0) {
                changedValues[key] = changedImages;
            }
        } else if (
            ...
            for (const nestedKey in currentData[key]) {
                if (
                    !isEqual(initialData[key]?.[nestedKey], currentData[key][nestedKey])
                ) {
  ...
  async function HandleSubmitFacility(e) {
    ...
    toast.dismiss();
    try {
      let facilityisownedbBDAValue =
        PostFacilityData.facilityisownedbBDA === "Yes" ? 1 : 0;
      console.log({ initialFacilityDataStore, PostFacilityData });
      const ChangesValues = getChnageValues(
        initialFacilityDataStore,
        PostFacilityData
      );
      ...
      let updatedFacilityImages = {
        facilityImageOne: ChangesValues.fileNames?.facilityImageOne
          ...
          : {},
        facilityArrayOfImages: ChangesValues.fileNames?.facilityArrayOfImages
      ...
      const payloadfacilityData = {
        ...
        fileNames: updatedFacilityImages || null,
        parkInventory:
          [
        ...
        // Consolidate ownersAddress data correctly
        ownersAddress: {

          ownershipDetailId: PostFacilityData.ownersAddress.ownershipDetailId || null,
          ...
          emailAddress: PostFacilityData.emailAdress || null,
          ownerPanCard: PostFacilityData.ownerPanCard || null,
          ownersAddress: PostFacilityData.ownersAddress.ownersAddress || null,
    ...
    if (files && files.length > 0) {
      if (name === "facilityImageOne") {
          ...
          reader.onloadend = () => {
            setPostFacilityData((prevState) => ({
              ...
              fileNames: {
                ...
                facilityImageOne: {
                  code: file.name,
                  fileId: prevState.fileNames.facilityImageOne?.fileId || null,
                  data: reader.result,
                },
              },
            }));
        ...
        const currentImagesCount =
          PostFacilityData.fileNames.facilityArrayOfImages.length;
        if (currentImagesCount + files.length > 5) {
          toast.warning(
            "You can only upload up to 5 images for Additional Facility Images."
          );
          document.getElementById("myForm").reset();
          return;
        }
        ...
        if (totalSize <= 1000) {
          validFiles.forEach((file) => {
            ...
            reader.onloadend = () => {
              setPostFacilityData((prevState) => {
                const newImages = [...prevState.fileNames.facilityArrayOfImages];
                // Check if the file already exists and update or add new
                const existingIndex = newImages.findIndex(
                  (img) => img.code === file.name
                );
                if (existingIndex !== -1) {
                  ...
                  newImages[existingIndex] = {
                    ...newImages[existingIndex],
                    data: reader.result,
                    fileId: newImages[existingIndex].fileId, // Retain existing fileId
                  };
                } else {
                  // Add new image
                  newImages.push({
                    code: file.name,
                    fileId: null, // New images get a null fileId
                    data: reader.result,
                  });
                }
                ...
                return {
                  ...
                  fileNames: {
                    ...prevState.fileNames,
                    facilityArrayOfImages: newImages,
                  },
                };
              });
          ...
          toast.warning(
            "Total size of images should not exceed 1000 KB for up to 5 images."
          );
          document.getElementById("myForm").reset();
        }
      }
  ...
  const handleImageRemove = (name, index) => {
    if (name === "facilityImageOne") {
      // Assuming `fileId` is available in `facilityImageOne`
      const fileId = PostFacilityData.fileNames.facilityImageOne.fileId;
      setPostFacilityData((prevState) => ({
        ...prevState,
        fileNames: {
          ...prevState.fileNames,
          facilityImageOne: {
            // Send fileId in the same structure
            code: "",
            fileId: fileId,
            data: "",
          },
  ...
  const handleRemoveRow = (index, e) => {
    e.preventDefault();
    let removedInventory = PostFacilityData.parkInventory.filter(
      (_, i) => i == index
    )[0];
              ...
              <div className="HostEvent_Heading">
                ...
                <div className="HostEvent_Row">
                  <div className="HostEvent_Group" id="AddressBox">
                    ...
                    />
                    {formErrors.address && (
                      <p className="error text-red-700">{formErrors.address}</p>
                    )}
                  </div>
                </div>
                <div className="HostEvent_Row">
                  <div className="HostEvent_Group">
                    ...
                    />
                    {formErrors.pin && (
                      <p className="error text-red-700">{formErrors.pin}</p>
                    )}
                  </div>
                  <div className="HostEvent_Group">
                    <label htmlFor="input2">
                    ...
                    />
                    {formErrors.area && (
                      <p className="error text-red-700">{formErrors.area}</p>
                    )}
                  </div>
                </div>
                <div className="HostEvent_Row">
                ...
                <div className="HostEvent_Row h-[120px]">
                  <div className="HostEvent_Group" id="AddressBox">
                    ...
                    {formErrors.service && (
                      <p className="error text-red-700">{formErrors.service}</p>
                    )}
                  </div>
                </div>
                <div className="HostEvent_Row">
                  <div className="HostEvent_Group" id="AddressBox">
                    <label htmlFor="input1">Other Services</label>
                    <input
                      type="massage"
                      id="input1"
                      className="input_padding"
                ...
                <div className="HostEvent_Row h-[170px]">
                  <div className="HostEvent_Group" id="AddressBox">
                    ...
                    {formErrors.amenity && (
                      <p className="error text-red-700">{formErrors.amenity}</p>
                    )}
                  </div>
                </div>
                <div className="HostEvent_Row">
                  <div className="HostEvent_Group" id="AddressBox">
                    <label htmlFor="input1">Other Amenities</label>
                    <input
                      type="massage"
                      id="input1"
                      className="input_padding"
                ...
                <div className="HostEvent_Row">
                  <div className="HostEvent_Group" id="AddressBox">
                    <label htmlFor="input1">Other Events Category</label>
                    <input
                      type="massage"
                      id="input1"
                      className="input_padding"
                ...
                {isPlayGround && (
                  <div className="HostEvent_Row">
                    <div className="HostEvent_Group" id="AddressBox">
                      ...
                      {formErrors.game && (
                        <p className="error text-red-700">{formErrors.game}</p>
                      )}
                    </div>
                  </div>
                )}
                {isPlayGround && (
                  <div className="HostEvent_Row">
                    <div className="HostEvent_Group" id="AddressBox">
                      <label htmlFor="input1">Other Games</label>
                      <input
                        type="massage"
                        id="input1"
                        className="input_padding"
                ...
                <form id="myForm" className="m-0">
                  <div className="container" id="facilityImageOneContainer">
                    {action == "Edit" && (
                      <div>
                        <h2 className="Upload_Image_text">
                    ...
                    {formErrors.facilityImageOne && (
                      <p className="error text-red-700  text-sm">
                        {formErrors.facilityImageOne}
                      </p>
                    )}
                        ...
                        <p>
                          {
                            ...
                            >
                              ...
                              <span
                                onClick={() =>
                                  handleImageRemove("facilityImageOne")
                                }
                                style={{
                                  cursor: "pointer",
                                  color: "red",
                  ...
                  <div
                    className="container"
                    id="facilityArrayOfImagesContainer"
                  >
                    {action == "Edit" && (
                      <div>
                        <h2 className="Upload_Image_text">
                          Upload Additional Facility Images
                        </h2>
                        <div className="upload-btn-wrapper">
                          <span>
                            {" "}
                          ...
                          <p className="italic text-sm font-bold text-gray-600">
                            You can only upload up to 5 image for Additional
                            Facility Images .
                          </p>
                        </div>
                      </div>
                    ...
                    {PostFacilityData.fileNames.facilityArrayOfImages && (
                      <div className="image-preview" id="imagePreview">
                        {PostFacilityData.fileNames.facilityArrayOfImages.map(
                          (image, index) => (
                            <span key={image.attachmentId}>
                              {image.code}
                              <span
                                onClick={() =>
                                  handleImageRemove("facilityArrayOfImages", index)
                                }
                                style={{
                                  cursor: "pointer",
                                  color: "red",
                  ...
                  <div className="Table_Inventory">
                    {action == "Edit" && (
                      <div className="Add_row_Button">
                        <button
                          className="Inventory_add_button"
                ...
                <div className="HostEvent_Row">
                  <div className="HostEvent_Group">
                    ...
                    >
                      ...
                      <option value="1" selected={PostFacilityData?.ownersAddress?.facilityisownedbBDA === 1}>
                        Yes
                      </option>
                      <option value="0" selected={PostFacilityData?.ownersAddress?.facilityisownedbBDA === 0}>
                        No
                      </option>
                    </select>
            ...
            {currentStep === 3 && (
              <form onSubmit={HandleSubmitFacility}>
                <div className="Verify_FromConatriner">
                  <div className="HostEvent_container">
                    <div className="HostEvent_Heading">
                      <h1 className="verify_name_text">
                        Step-3 (Verify facility details before submit)
                      </h1>
                      <div className="HeadingTitle9">
                        <div></div>
                        <h2>Facility details</h2>
                      ...
                      <div className="HostEvent_Row">
                        <div className="HostEvent_Group">
                          <label htmlFor="input1">Facility Type </label>
                          <select
                            id="input2"
                            className="input_padding"
                            name="facilityType"
                        ...
                        <div className="HostEvent_Group">
                          <label htmlFor="input2">Facility Name </label>
                          <input
                            type="text"
                            id="input2"
                            className="input_padding"
                      ...
                      <div className="HostEvent_Row">
                        ...
                        <div className="HostEvent_Group">
                          <label htmlFor="input2">Latitude</label>
                          <input
                            type="text"
                            id="input2"
                            className="input_padding"
                      ...
                      <div className="HostEvent_Row">
                        <div className="HostEvent_Group">
                          <label htmlFor="input1">Pincode </label>
                          <input
                            type="text"
                            className="input_padding"
                            id="input1"
                        ...
                        <div className="HostEvent_Group">
                          <label htmlFor="input2">Area Acre </label>
                          <input
                            type="text"
                            id="input2"
                            className="input_padding"
                      ...
                      <div className="HostEvent_Row">
                        <div className="HostEvent_Group">
                          <label htmlFor="input1">Operating From Time </label>
                          <input
                            type="time"
                            className=" p-5"
                            placeholder="Enter Pin  "
                        ...
                        <div className="HostEvent_Group">
                          <label htmlFor="input2">Operating To Time</label>
                          <input
                            type="time"
                            id="input2"
                            className="input_padding p-5"
                      ...
                      <div className="HostEvent_Row">
                        <div className="HostEvent_Group" id="AddressBox">
                          <label htmlFor="input1">Operating Days</label>
                          <span className="Operating_day" name="operatingDays">
                            <button
                              type="button"
                              className={`button-4 ${PostFacilityData.operatingDays.sun
                                ? "selected"
                                : ""
                                }`}
                              onClick={(e) => handleDayClick("sun", e)}
                            ...
                            <button
                              type="button"
                              className={`button-4 ${PostFacilityData.operatingDays.mon
                                ? "selected"
                                : ""
                                }`}
                              onClick={(e) => handleDayClick("mon", e)}
                            ...
                            <button
                              type="button"
                              className={`button-4 ${PostFacilityData.operatingDays.tue
                                ? "selected"
                                : ""
                                }`}
                              onClick={(e) => handleDayClick("tue", e)}
                            ...
                            <button
                              type="button"
                              className={`button-4 ${PostFacilityData.operatingDays.wed
                                ? "selected"
                                : ""
                                }`}
                              onClick={(e) => handleDayClick("wed", e)}
                            ...
                            <button
                              type="button"
                              className={`button-4 ${PostFacilityData.operatingDays.thu
                                ? "selected"
                                : ""
                                }`}
                              onClick={(e) => handleDayClick("thu", e)}
                            ...
                            <button
                              type="button"
                              className={`button-4 ${PostFacilityData.operatingDays.fri
                                ? "selected"
                                : ""
                                }`}
                              onClick={(e) => handleDayClick("fri", e)}
                            ...
                            <button
                              type="button"
                              className={`button-4 ${PostFacilityData.operatingDays.sat
                                ? "selected"
                                : ""
                                }`}
                              onClick={(e) => handleDayClick("sat", e)}
                              ...
                              GetServiceData.map((item, index) => (
                                <span key={index}>
                                  <button
                                    type="button"
                                    onClick={() =>
                                      handleServiceClick(item.serviceId)
                                    }
                                    disabled
                                    className={`button-4 ${PostFacilityData.service.includes(
                                      item.serviceId
                      ...
                      <div className="HostEvent_Row">
                        <div className="HostEvent_Group" id="AddressBox">
                          <label htmlFor="input1">Amenities</label>
                          <span className="Operating_day">
                            {GetAmenitiesData?.length > 0 &&
                              GetAmenitiesData?.map((data, index) => (
                                <span key={index}>
                      ...
                      <div className="HostEvent_Row">
                        <div className="HostEvent_Group" id="AddressBox">
                          <label htmlFor="input1">About the Facility</label>
                          <input
                            type="massage"
                            id="input1"
                            className="input_padding"
                      ...
                      <div className="Pakr_inventory_form_main_Conatiner">
                        <h1 className="Park_Inventory_text">Park Inventory</h1>
                        <div className="Table_Inventory">
                          <div className="Add_row_Button"></div>
                          <table className="Inventory_table">
                            <thead className="Inventory_thead">
                              <tr className="Inventory_tr">
                                <th className="Inventory_th">
                                  Items/Equipment
                                </th>
                                <th className="Inventory_th">Number</th>
                              </tr>
                            ...
                            <tbody className="Inventory_tbody">
                              {PostFacilityData.parkInventory.map(
                                (item, index) => (
                                  <tr className="Inventory_tr" key={index}>
                                    <td className="Inventory_td">
                                      <select
                    ...
                    <div className="HostEvent_Heading">
                      ...
                      <div className="HostEvent_Row">
                        <div className="HostEvent_Group">
                          ...
                          {formErrors.facilityisownedbBDA && (
                            <p className="error text-red-700">
                              {formErrors.facilityisownedbBDA}
                            </p>
                          )}
                      ...
                      <div className="HostEvent_Row">
                        ...
                        <div className="HostEvent_Group">
                          <label htmlFor="input2">Email Adress </label>
                          <input
                            type="text"
                            id="input2"
                            className="input_padding"
                      ...
                      <div className="HostEvent_Row">
                        ...
                        <div className="HostEvent_Group">
                          <label htmlFor="input1">Ownership </label>
                          <input
                            type="text"
                            className="input_padding"
                            id="input1"
                      ...
                      <div className="HostEvent_Row">
                        <div className="HostEvent_Group" id="AddressBox">
                          <label htmlFor="input1">Owner’s Address</label>
                          <input
                            type="massage"
                            id="input1"
                            className="input_padding"
