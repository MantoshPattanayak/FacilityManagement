...
export default function Details() {
  ...
  async function getEventDetailsById(eventId) {
    console.log('eventId in fetch event details', eventId);
    try {
      ...
      console.log("here Responseget sub data of ", res);
      // setServiceData(res.data.serviceData);
      // setAmenitiesData(res.data.amenitiesData);
      setEventAvailable(res.data.eventActivityDetails);
  ...
  return (
    <div className="event-Sub_Manu_Conatiner">
      ...
      <div className="event-map_img_main_conatiner">
        ...
        <div className="eventmap-parent">
        <div className="event-Map_container">
          <span className="event-time_status">
            <h1 className="event-date_text">
              {""}
              Date :{formatDate(EventAvailable?.eventDate)}
            </h1>

            <h1 className='event-time_text'>
              
              Timing : {formatTime(
              EventAvailable?.eventStartTime
            )} - {formatTime(EventAvailable?.eventEndTime)}</h1>
          ...
          {amenitiesData
            ...
            .map((item, index) => (
              <span className="flex gap-2" key={index}>
                ...
                />
                <h1 className="event-Amenities_name">{item.eventName}</h1>
              </span>
            ))}
        </div>
      </div> */}
      ...
      <div className="event-About_Conatiner">
        ...
        <h1 className="event-About_text">
          {/* {EventAvailable?.length > 0 && EventAvailable?.descriptionOfEvent} */}
          {EventAvailable.descriptionOfEvent}
        </h1>
      </div>
      {/* -------------------------Helpline Number ------------------------------------------ */}
      <div className="event-Helpline_number_conatine">
        ...
        <div className="event-Contact_number">
          ...
          <h1 className="event-Number">987986696</h1>
        </div>
      </div>
      {/* -------------------------Event Available ----------------------------------------------------------- */}
                ...
                >
                  <div className="event-carousel-slide">
                    <img
                      ...
                      alt="Event"
                    ></img>
                    <h1 className="event-Name_yoga"> {eventActivityDetails.eventName}</h1>
